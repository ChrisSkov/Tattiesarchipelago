...
{
    public WeaponObj currentWeapon;
    public WeaponObj leftHand;
    public WeaponObj rightHand;
    public PlayerStats stats;

    [SerializeField] public GameObject[] weapons = new GameObject[3];
    Animator anim;
    AudioSource source;
    ...
    void PunchAnimEvent()
    {
        ...
        foreach (Collider c in Physics.OverlapSphere(weapons[0].transform.position, currentWeapon.attackRange, mask))
        {
            if (CriticalHit())
            {
                c.gameObject.GetComponent<EnemyHealth>().TakeDamage(currentWeapon.attackDamage * stats.critDamageMultiplier);
                var clone = Instantiate(stats.critEffect, weapons[0].transform.position, weapons[0].transform.rotation);
                Destroy(clone, 0.3f);
            }
            else
            {
                c.gameObject.GetComponent<EnemyHealth>().TakeDamage(currentWeapon.attackDamage);
            }
    ...
    void RightPunchAnimEvent()
    {
        RaycastHit hit;
        LayerMask mask = LayerMask.GetMask("Enemy");
        foreach (Collider c in Physics.OverlapSphere(weapons[2].transform.position, currentWeapon.attackRange, mask))
        {
            c.gameObject.GetComponent<EnemyHealth>().TakeDamage(currentWeapon.attackDamage);
    ...
    void ThrowObjectAnimEvent()
    {
        var clone = Instantiate(currentWeapon.wepPrefab, weapons[2].transform.position, weapons[2].transform.rotation);
        clone.GetComponent<Rigidbody>().AddForce(transform.TransformDirection(transform.forward) * 15, ForceMode.Impulse);
        source.PlayOneShot(stats.throwSounds[Random.Range(0, stats.throwSounds.Length)]);
